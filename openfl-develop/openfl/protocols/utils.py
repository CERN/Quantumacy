...
def decrypt_AES(cypher, message):
    message = cypher.decrypt(message.encode())

    value = ast.literal_eval(message.decode())
    if isinstance(value, dict):  # not safe
        class Resp:
            pass
...
def encrypt_AES(cypher, message):
    if hasattr(message, '__dict__'):
        message = repr(message.__dict__)
        final = cypher.encrypt(message.encode()).decode()
    else:
        message = repr(message).encode()
        final = cypher.encrypt(message).decode()
    return final


def decrypt_named_tensor(payload, cypher):
    payload = decrypt_AES(cypher, payload)
    transformer_metadata = decrypt_metadata(payload.transformer_metadata, cypher)
    data_bytes = decrypt_AES(cypher, payload.data_bytes)
    named_tensor = NamedTensor(name=payload.name,
...
def decrypt_metadata(metadata, cypher):
    metadata = decrypt_AES(cypher, metadata)
    list_metadata = []
    for i in metadata:
        int_list = []
        bool_list = []
        int_to_float = {}
        for il in i['int_list']:
            int_list.append(decrypt_AES(cypher, il))
        for bl in i['bool_list']:
            bool_list.append(decrypt_AES(cypher, bl))
        for k, v in zip(i['int_to_float_keys'], i['int_to_float_values']):
            int_to_float[decrypt_AES(cypher, k)] = decrypt_AES(cypher, v)

        list_metadata.append(MetadataProto(int_list=int_list, bool_list=bool_list, int_to_float=int_to_float))
    return list_metadata


def encrypt_named_tensor(named_tensor, cypher):
    transformer_metadata = encrypt_metadata(named_tensor.transformer_metadata, cypher)
    data_bytes = encrypt_AES(cypher, named_tensor.data_bytes)

    class Payload:
        def __init__(self):
            self.name = named_tensor.name
            self.round_number = named_tensor.round_number
            self.lossless = named_tensor.lossless
            self.report = named_tensor.report
            self.tags = named_tensor.tags
            self.transformer_metadata = transformer_metadata
            self.data_bytes = data_bytes

    return encrypt_AES(cypher, Payload())


def encrypt_metadata(metadata, cypher):
    ...
    for i in metadata:
        ...
        int_to_float_keys = []
        for il in i.int_list:
            int_list.append(encrypt_AES(cypher, il))
        for bl in i.bool_list:
            bool_list.append(encrypt_AES(cypher, bl))
        for itfv in i.int_to_float.values():
            int_to_float_values.append(encrypt_AES(cypher, itfv))
        for itfk in i.int_to_float.keys():
            int_to_float_keys.append(encrypt_AES(cypher, itfk))

        enc_metadata = {'int_list': int_list,
                        'bool_list': bool_list,
                        'int_to_float_values': int_to_float_values,
                        'int_to_float_keys': int_to_float_keys}

        list_metadata.append(enc_metadata)

    return encrypt_AES(cypher, list_metadata)
